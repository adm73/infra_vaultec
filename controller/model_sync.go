...
// 上游地址
const (
	upstreamModelsURL  = "https://basellm.github.io/llm-metadata/api/vaultec/models.json"
	upstreamVendorsURL = "https://basellm.github.io/llm-metadata/api/vaultec/vendors.json"
)
...
func getUpstreamURLs(locale string) (modelsURL, vendorsURL string) {
	...
	if l, ok := normalizeLocale(locale); ok && l != "" {
		return fmt.Sprintf("%s/api/i18n/%s/vaultec/models.json", base, l),
			fmt.Sprintf("%s/api/i18n/%s/vaultec/vendors.json", base, l)
	}
	return fmt.Sprintf("%s/api/vaultec/models.json", base), fmt.Sprintf("%s/api/vaultec/vendors.json", base)
}
