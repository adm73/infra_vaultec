...
// are exposed separately for self-service flows driven from vaultec's own UI.
...
func WaffoPancakeBuyerIdentityFromUserID(userID int) string {
	return fmt.Sprintf("vaultec-user-%d", userID)
}
...
const (
	defaultWaffoPancakeStoreName   = "vaultec-store"
	defaultWaffoPancakeProductName = "vaultec-charge-product"
)
...
//
// OnetimeProduct (not SubscriptionProduct) because vaultec has no renewal-
// event handling; Pancake auto-renewing without vaultec extending user
