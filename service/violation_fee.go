...
func HasCSAMViolationMarker(err *types.VaultecError) bool {
	if err == nil {
		return false
	}
...
func WrapAsViolationFeeGrokCSAM(err *types.VaultecError) *types.VaultecError {
	if err == nil {
		return nil
	}
...
func NormalizeViolationFeeError(err *types.VaultecError) *types.VaultecError {
	if err == nil {
		return nil
	}
...
func shouldChargeViolationFee(err *types.VaultecError) bool {
	if err == nil {
		return false
	}
...
func ChargeViolationFeeIfNeeded(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, apiErr *types.VaultecError) bool {
