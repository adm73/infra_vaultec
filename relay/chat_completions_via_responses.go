...
import (
	...
	openaichannel "github.com/adm73/infra_vaultec/relay/channel/openai"
...
func chatCompletionsViaResponses(c *gin.Context, info *relaycommon.RelayInfo, adaptor channel.Adaptor, request *dto.GeneralOpenAIRequest) (*dto.Usage, *types.VaultecError) {
	...
	if len(info.ParamOverride) > 0 {
		...
		if err != nil {
			return nil, vaultecErrorFromParamOverride(err)
		}
	}
	...
	if httpResp.StatusCode != http.StatusOK {
		vaultecErr := service.RelayErrorHandler(c.Request.Context(), httpResp, false)
		service.ResetStatusCode(vaultecErr, statusCodeMappingStr)
		return nil, vaultecErr
	}

	if upstreamStream && clientStream {
		usage, vaultecErr := openaichannel.OaiResponsesToChatStreamHandler(c, info, httpResp)
		if vaultecErr != nil {
			service.ResetStatusCode(vaultecErr, statusCodeMappingStr)
			return nil, vaultecErr
		}
	...
	if upstreamStream {
		info.IsStream = false
		usage, vaultecErr := openaichannel.OaiResponsesToChatBufferedStreamHandler(c, info, httpResp)
		if vaultecErr != nil {
			service.ResetStatusCode(vaultecErr, statusCodeMappingStr)
			return nil, vaultecErr
		}
	...
	usage, vaultecErr := openaichannel.OaiResponsesToChatHandler(c, info, httpResp)
	if vaultecErr != nil {
		service.ResetStatusCode(vaultecErr, statusCodeMappingStr)
		return nil, vaultecErr
	}
